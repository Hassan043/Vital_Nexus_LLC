...
| Item | How |
|------|-----|
| Page titles / welcome text | Language customization (`en`) — template in [`branding/en.default-strings.template.json`](branding/en.default-strings.template.json) |
| Logo, banner, colors | Azure portal → **Company branding** on the external tenant |
| Layout | Azure portal → User flows → **Page layouts** |
...
- **F3.T1.5** — MFA, password reset, environment settings / Key Vault sync
